...
{
    ConsoleLog.print(F("[XTouch][SD] Writting JSON file: "));
    ConsoleLog.println(filename);
    File configFile = fs.open(filename, FILE_WRITE);
    if (!configFile)
    {
        ConsoleLog.print(F("[XTouch][SD] Failed to write json file: "));
        ConsoleLog.println(filename);
        return;
    }
...
{
    ConsoleLog.print(F("[XTouch][SD] Reading JSON file: "));
    ConsoleLog.println(filename);
    DynamicJsonDocument doc(size); // Adjust the size as needed

    if (!fs.exists(filename))
    {
        ConsoleLog.print(F("[XTouch][SD] Error Reading JSON File: "));
        ConsoleLog.println(filename);
        if (defaultsToArray)
        {
    ...
    if (error)
    {
        ConsoleLog.print(F("[XTouch][SD] Error Parsing JSON File: "));
        ConsoleLog.println(filename);
    }
